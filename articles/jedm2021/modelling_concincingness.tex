...
\begin{itemize}
	...
	\item Semantic:
	we calculate similarity metrics to i) all other explanations, ii) the 
	question item text, and, when available, iii) a teacher provided ``expert'' 
	explanation, using pre-trained GloVe \cite{pennington_glove:_2014} vectors.
...
of the answers for all other question items in the same discipline, which 
should capture discipline specific patterns while addressing a the 
``cold-start'' problem for new items before vote data can be collected.
...
preference data.
More words typically means more sound argumentation, and pairs where the same 
logical argument is presented, but one more concisely than the other, are not 
enough to counter this.
Therefore, when training our feature-rich models to learn to predict the 
real-valued rank scores, we dis-aggregate by word-count as well. 
...
\textit{precision@K}; e.g. how many of the top-5 Elo-ranked explanations (in 
