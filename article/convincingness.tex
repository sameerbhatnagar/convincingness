...
\email{\{first.author,second.author,third.author\}@inst1.edu}\and
Second Institute
\email{second.author@inst2.edu}\\
}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\todo{Must be anonymized before submission}
...
We filter the dataset to only include topics that have at least 50 associated 
argument pairs.
...
either from the wrong to right, or the reverse. 
%These pairs add a different level of complexity that models must learn, and 
%are 
...
In this respect, our the \textbf{xxxxxxxx} dataset is different than the 
...
In our study, three models are defined and compared, and are described below.
...
\begin{figure}
	...
	\caption{Pairwise ranking classification accuracy and ROC-AUC for 
		...
		\textbf{xxxxxxxx} dataset, across disciplines. All results evaluated 
		using 
...
not uniform across the datasets, which, we surmise, is due in most part to how carefully 
...
curated, 
and contain many more argument pairs that have a larger relative 
...
highlight the impact of the availability of the current topic data for the training.
When evaluating performance using 5-fold cross-validation (figure~\ref{fig:performance_k_fold}), where the topic data is held-in for the training,
