...
def argLength():
    ...
    for data_source in data_sources:
      data[data_source] = data_loaders.load_arg_pairs(data_source,cross_topic_validation=CROSS_TOPIC)

    data["dalite"] = data_loaders.load_dalite_data(cross_topic_validation=CROSS_TOPIC)
    data["IBM_Evi"] = data_loaders.load_arg_pairs_IBM_Evi(cross_topic_validation=CROSS_TOPIC)
    ...
    for i,(df_train,df_test) in enumerate(zip(train_dataframes,test_dataframes)):
      ...
      X_train=np.array(list(zip(df_train["a1_wc"].values,df_train["a2_wc"].values)))
      y_train=df_train["y"]
    ...
    means_argLength_dalite=df_test_all.groupby(
        ["discipline","question"]
        ).apply(
            weighted_scorers.my_scores
            ).groupby("discipline").apply(
                weighted_scorers.weighted_avg
                ).round(2)

    means_argLength_dalite["dataset"]="dalite"
    means_argLength_dalite=means_argLength_dalite.reset_index()

    means_argLength_dalite_plot=melt_results(means_argLength_dalite,id_var="discipline")
    ...
    for data_source in data.keys():
      ...
      means_argLength=df_test_all.groupby(["topic"]).apply(weighted_scorers.my_scores)
      means_argLength["dataset"]=data_source
      means_all = pd.concat([means_all,means_argLength])

    means_argLength_other=means_all.groupby("dataset").apply(weighted_scorers.weighted_avg).round(2)
    means_argLength=pd.concat([
               means_argLength,
               means_argLength_other.reset_index()
    ])
...
def assemble_results():
    ...
    # ArgBoW
    print("ArgBow")
    fname=os.path.join(data_loaders.BASE_DIR,"tmp","df_results_all_ArgBow_cross_topic_validation_{}.csv".format(CROSS_TOPIC))
    df_results_all = pd.read_csv(fname)

    means=df_results_all.groupby(["dataset","topic"]).apply(weighted_scorers.my_scores)
    means_df_bow=means.groupby("dataset").apply(weighted_scorers.weighted_avg).round(2).reset_index()
    ...
    means_dalite=df_results_all_dalite.groupby(["discipline","topic"]).apply(weighted_scorers.my_scores)
    means_df_bow_dalite=means_dalite.groupby("discipline").apply(weighted_scorers.weighted_avg).round(2).reset_index()

    means_df_plot_dalite_disc_bow= melt_results(means_df_bow_dalite,id_var="discipline")
    means_df_plot_dalite_disc_bow["model"]="ArgBoW"

    means_argLength_argBoW_plot_dalite=pd.concat([means_argLength_dalite_plot,means_df_plot_dalite_disc_bow])
    ...
    for data_source in ["IBM_ArgQ","IBM_Evi","UKP","dalite"]:
        fname=os.path.join(
            ...
            "tmp",
            "df_test_all_ArgGlove_cross_topic_validation_True_{}.csv".format(data_source)
        )
    ...
    means_glove=df_results_all.groupby(["dataset","topic"]).apply(weighted_scorers.my_scores)
    means_df_glove=means_glove.groupby("dataset").apply(weighted_scorers.weighted_avg).round(2).reset_index()
    ...
    df_results_all_dalite=df_results_all[df_results_all["dataset"]=="dalite"]
    means_dalite_glove=df_results_all_dalite.groupby(["discipline","topic"]).apply(weighted_scorers.my_scores)
    means_df_glove_dalite=means_dalite_glove.groupby("discipline").apply(weighted_scorers.weighted_avg).round(2).reset_index()

    means_df_plot_dalite_disc_glove= melt_results(means_df_glove_dalite,id_var="discipline")
    means_df_plot_dalite_disc_glove["model"]="ArgGlove"
    means_df_plot_dalite_disc_glove=pd.concat([means_argLength_argBoW_plot_dalite,means_df_plot_dalite_disc_glove])
    ...
    for data_source in ["IBM_ArgQ","IBM_Evi","UKP"]:
      results_dir = os.path.join(data_loaders.BASE_DIR,"tmp","results_cross_topic","BERT_{}_results".format(data_source))
      for fold in range(len(os.listdir(results_dir))):
        fname=os.path.join(results_dir,"fold-{}".format(fold),"df_test-fold-{}.csv".format(fold))
    ...
    for discipline in ["Biology","Chemistry","Physics"]:
      results_dir = os.path.join(data_loaders.BASE_DIR,"tmp","results_cross_topic","BERT_{}_results".format(data_source),discipline)
      for fold in range(len(os.listdir(results_dir))):
        fname=os.path.join(results_dir,"fold-{}".format(fold),"df_test-fold-{}.csv".format(fold))
        df_test = pd.read_csv(fname)
        df_test["dataset"]=data_source
    ...
    df_results_bert_all["pred_score_1_soft"]=softmax(df_results_bert_all[["pred_score_0","pred_score_1"]].values,axis=1)[:,1]

    df_results_bert_all.groupby("dataset").size()

    means_bert=df_results_bert_all.groupby(["dataset","topic"]).apply(weighted_scorers.my_scores)
    means_df_bert=means_bert.groupby("dataset").apply(weighted_scorers.weighted_avg).round(2).reset_index()
    ...
    df_results_bert_all_dalite=df_results_bert_all[df_results_bert_all["dataset"]=="dalite"]
    means_dalite_bert=df_results_bert_all_dalite.groupby(["discipline","topic"]).apply(weighted_scorers.my_scores)
    means_df_bert_dalite=means_dalite_bert.groupby("discipline").apply(weighted_scorers.weighted_avg).round(2).reset_index()

    means_df_plot_dalite_disc_bert= melt_results(means_df_bert_dalite,id_var="discipline")
    means_df_plot_dalite_disc_bert["model"]="ArgBERT"
    # means_df_plot_dalite_disc_bert
    means_df_plot_dalite_disc=pd.concat([means_df_plot_dalite_disc_glove,means_df_plot_dalite_disc_bert])
