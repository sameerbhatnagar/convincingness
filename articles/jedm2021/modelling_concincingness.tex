...
\begin{document}
	...
	that enrich 
	the bank of learning materials available to their peers, all as an 
	authentic part of their own learning experience.  
...
discussed in \citeN{gagnon_filtering_2019}, but are limited to simply ensuring 
...
%We suggest that the ``vote'' data collected for each student's 
...
proposed by \citeN{habernal_which_2016}: if crowd-workers are presented with a 
...
different teachers at different colleges and universities in Canada. 
...
labelled pairs, we apply the following rank aggregation 
techniques in order to derive a real valued \textit{convincingness} rank score 
for each student explanation, as depicted in figure \ref{fig:make_pairs_a}(c).
...
\begin{enumerate}
	...
	towards including the effect of comparative judgement, as pairs are 
	...
	JuxtaPeer \cite{cambre_juxtapeer:_2018}.\footnote{Implementation taken from 
...
step, wherein the top 768 most discriminating features are retained, and used 
as input for the classical regression models described earlier.  
...
which is a requirement for estimating the annotator-specific $\eta_k$).
...
employ a validation scheme similar to one proposed by \citeN{jones_peer_2015}.
...
\textit{feature importance} introduced by \citeN{breiman_random_2001} for 
random forests, and generalized to be model agnostic by 
\citeN{fisher_all_2019}: each feature is randomly permuted for a set number of 
repetitions, and the importance of that feature is measured by the average 
decrease in performance of the model on the un-perturbed dataset.
...
\begin{table}
	...
	\parbox{.45\linewidth}{
		\centering
		\input{data/ranking_y_BT_pearson}
		\subcaption{Pearson correlation}
	}
	\hfill
	\parbox{.45\linewidth}{
		\centering
		\input{data/ranking_y_BT_spearman}
		\subcaption{Spearman correlation}
	}
\end{table}
...
\begin{table}
	...
	\parbox{.45\linewidth}{
		\centering
	\input{data/ranking_dalite_y_winrate_pearson}
		\subcaption{Pearson correlation}
	}
	\hfill
	\parbox{.45\linewidth}{
...
\begin{table}
	...
	\parbox{.45\linewidth}{
		\centering
		\input{data/ranking_dalite_y_BT_pearson}
		\subcaption{Pearson correlation}
	}
	\hfill
	\parbox{.45\linewidth}{
		\centering
		\input{data/ranking_dalite_y_BT_spearman}
		\subcaption{Spearman correlation}
	}
\end{table}
...
\begin{table}
	...
	\parbox{.45\linewidth}{
		\centering
		\input{data/ranking_dalite_y_crowdBT_pearson}
		\subcaption{Pearson correlation}
	}
	\hfill
	\parbox{.45\linewidth}{
		\centering
		\input{data/ranking_dalite_y_crowdBT_spearman}
		\subcaption{Spearman correlation}
	}
...
\begin{table}
	...
	\parbox{.45\linewidth}{
		\centering
		\input{data/ranking_dalite_y_winrate_nopairs_pearson}
		\subcaption{Pearson correlation}
	}
	\hfill
	\parbox{.45\linewidth}{
	\centering
		\input{data/ranking_dalite_y_winrate_nopairs_spearman}
		\subcaption{Spearman correlation}
	}
\end{table}
