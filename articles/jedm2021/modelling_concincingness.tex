...
list is using the Bradley-Terry model \cite{bradley_rank_1952}, which has also 
been specifically proposed for ordinal peer grading 
data\cite{raman_methods_2014}.
...
\begin{enumerate}
	...
	The rating update rule transfers ``strength'' from the winner, to the 
	loser, in proportion to the difference in strength:
...
How we evaluate the fit of these ran aggregation methods to our data is 
described in section \ref{sec:methodology_eval}

\subsection{Modelling rank scores}
...
In order to evaluate the generalizability of our models to new question items, 
we employ a ``cross-topic'' cross-validation scheme, wherein we hold out all of 
the answers on one question item train models on all of the answers for all 
question items except one
...
included here are cohesion, sentiment, and psycholinguistic features, as they 
...
In order to evaluate our choice of rank aggregation scores, and address 
our research question RQ1, we perform several validation tests.

Firstly, the reference argument mining datasets that we use for this study, 
each include some their own derived aggregated rank score (described in 
\ref{sec:related_work:arg_quality}) for each argument.
We evaluate the soundness of our choice of simpler rank aggregation methods by 
...
correlation between the ``reference'' score each argument and our simpler 
scores.
The distribution of Pearson correlation coefficients across the different 
topics are shown in the boxplots in figure \ref{fig:corr_to_reference_score}, 
...
\begin{figure}[H]
	...
	``reference'' rank scores, and the rank aggregation methods (WinRate, BT, 
	Elo) used in our proposed methodology, across the different topics of the 
	reference argument mining datasets.}
	\label{fig:corr_to_reference_score}
\end{figure}

In order to evaluate the reliability of these rankings we employ a validation 
scheme similiar to one proposed by \cite{jones_peer_2015}.
Students are randomly split into two batches, and their answers ...

\begin{figure}[H]
	\centering
	\scalebox{0.5}{\input{img/evaluate_rankings.pdf_tex}}
	\caption{
		Evaluating of reliability of rank scores: for each question, student 
		answers are divided into two batches, yielding two batches of 
		corresponding pairs, and two aggregated rankings.
		Two measures of reliability of the derived rankings are shown with the 
		yellow arrows: 
		i) the rank scores of each batch can be used to predict the pairwise 
		preferences of the other batch of students, and 
		ii) the Kendall tau correlation coefficient can be calculated between 
		the two independantly derived ranked lists for each batch of students. 
	}
...
\section{Results}\label{sec:model_results}
