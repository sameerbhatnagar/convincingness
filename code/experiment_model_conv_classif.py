...
def main(
    ...
    for target in targets:
        ...
        for t,topic in enumerate(topics):
            df_train=df2[(df2["topic"]!=topic)&(df2["wc_bin"].isin(quartiles))].dropna(subset=[target])
            y_train=df_train[target]
            X_train_df=df_train.drop(drop_cols+["id"],axis=1)

            df_test=df2[(df2["topic"]==topic)&(df2["wc_bin"].isin(quartiles))].dropna(subset=[target])
            ...
            for clf_name,clf in clfs:
                ...
                if clf_name == "length":
                    if task=="classif":
                        df_train=df[
                            (df["topic"]!=topic)&(df["wc_bin"].isin(quartiles))
                ...
                if task=="classif":
                    ...
                    d["pearsonr"]=pearsonr(y_pred,y_test)
                    d["spearmanr"]=spearmanr(y_pred,y_test)
    ...
    fp=os.path.join(
        ...
        f"pred_rank_score_{task}_{discipline}_{population}.pkl"
    )
