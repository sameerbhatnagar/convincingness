...
#     )
    feature_types.extend(
        ["ngram" for i in range(len(pipe[0].named_transformers_["ngram"].get_feature_names()))]
    )
    feature_types.extend(
        ["pos" for i in range(len(pipe[0].named_transformers_["pos"].get_feature_names()))]
    )
    feature_types.extend(
        ["chunk_dep" for i in range(len(pipe[0].named_transformers_["chunk_dep"].get_feature_names()))]
    )
    feature_types.extend(
        ["text_pos" for i in range(len(pipe[0].named_transformers_["text_pos"].get_feature_names()))]
    )
    feature_names = list(compress(
...
def main(
    ...
    for target in targets:
        for topic in topics:
            conv_thresh_ratio = np.quantile(df[df["topic"]==topic][target],q=CONV_THRESH_RATIO)
            df.loc[(
                (df[target]>=conv_thresh_ratio)
                &(df["topic"]==topic)
            ),f"{target}_bin"]=1
            df.loc[(
                (df[target]<conv_thresh_ratio)
    ...
    targets_bin=[f"{t}_bin" for t in targets]

    features_pipe =ColumnTransformer([
        ("scale",StandardScaler(),numeric_columns),
#             ("one_hot",OneHotEncoder(drop=["rw"]),categorical_columns),
        ("ngram",TfidfVectorizer(use_idf=True,),"rationale_content_words"),
        ("pos",TfidfVectorizer(use_idf=False),"rationale_pos"),
        ("chunk_dep",TfidfVectorizer(use_idf=False,analyzer=white_space_analyzer),"rationale_chunk_dependencies"),
        ("text_pos",TfidfVectorizer(use_idf=False,analyzer=white_space_analyzer),"rationale_text_pos"),
    ])
    ...
    #         +categorical_columns
        ...
        topics=df2["topic"].value_counts().index.to_list()

        for t,topic in enumerate(topics):
            df_train=df2[(df2["topic"]!=topic)&(df2["wc_bin"]==quartile)].dropna(subset=[target])
            y_train=df_train[target]
            X_train_df=df_train.drop(drop_cols+["id"],axis=1)

            df_test=df2[(df2["topic"]==topic)&(df2["wc_bin"]==quartile)].dropna(subset=[target])
            ...
            for clf_name,clf in clfs:
                ...
                if clf_name == "length":
                    df_train=df[
                        (df["topic"]!=topic)&(df["wc_bin"]==quartile)
                    ].dropna(subset=[target])["rationale"].str.count("\w+").values.reshape(-1,1)
                    clf = LogisticRegression()
                    clf.fit(df_train,y_train)
                    y_pred = clf.predict(
                        X_test_df["rationale_content_words"].str.count("\w+").values.reshape(-1,1)
                    )
                    ...
                    important_features = get_important_feature_names(
                        features_pipe_full,clf,numeric_columns,X_test.toarray(),y_test)
    ...
    fp=os.path.join(
        ...
        f"pred_rank_score_composite_features_classification_{discipline}_{population}.pkl"
    )
